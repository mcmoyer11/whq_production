...
d_subs = d %>%
  group_by(subject) %>%
  filter(Nat_lang %in% c("Bengali","Spanish","arabic"))

nrow(d_subs)
levels(d$Nat_lang)

nn = subset(d, (d$Nat_lang == "Bengali" | d$Nat_lang == "Spanish" | d$Nat_lang == "arabic"))
nnb = nn %>%
  filter(Nat_lang %in% c("arabic"))
nrow(nnb)
View(nnb)
...
# X-squared = 222.09, df = 1, p-value < 2.2e-16
